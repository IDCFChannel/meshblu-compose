...
def on_message(client, userdata, msg):
    ...
    if data:
        ...
        if retval:
            print("-"*10)
            print("objctTemperature: {}".format(retval["objctTemperature"]))
            if float(retval["objctTemperature"]) > conf["THRESHOLD"]:
                print("threshold over: {0} > {1}".format(float(retval["objctTemperature"]),
                                                         conf["THRESHOLD"]))
